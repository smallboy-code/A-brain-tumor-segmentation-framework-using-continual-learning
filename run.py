...
def main(opts):
    ...
    for i, (step, lr) in enumerate(zip(copy.deepcopy(opts.step), copy.deepcopy(opts.lr))):
        ...
        val_score = run_step(opts, device)
        #if rank == 0:
...
def run_step(opts, device):
    # Initialize logging
    task_name = f"{opts.task}-{opts.dataset}"
    logdir_full = f"{opts.logdir}/{task_name}/{opts.name}/"


    logger = Logger(logdir_full, rank=0, debug=opts.debug, summary=False)
    ...
    if model_old is not None:
        model, model_old, optimizer = model.to(device), model_old.to(device), optimizer
    else:
        model, optimizer = model.to(device), optimizer
