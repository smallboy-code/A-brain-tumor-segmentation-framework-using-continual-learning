...
class VOCSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.load(self.images[index][0]).astype(np.float32)
        #target = Image.open(self.images[index][1])
        target = np.load(self.images[index][1]).astype(np.uint8)
    ...
    def viz_getter(self, index):
        ...
        raw_image = np.load(self.images[index][0]).astype(np.float32)
        #target = Image.open(self.images[index][1])
        target = np.load(self.images[index][1]).astype(np.uint8)
        if self.transform is not None:
