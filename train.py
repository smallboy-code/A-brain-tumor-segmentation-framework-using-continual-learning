    ...
    def validate(self, loader, metrics, ret_samples_ids=None, logger=None, end_task=False):
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                # imas = images.cpu().numpy()
                labels = labels.cpu().numpy()
                prediction = prediction.cpu().numpy()
                # np.save(fr'./results/images/img_{i}.npy', imas)
                # np.save(fr'./results/pred1/pred_{i}.npy',prediction)
                # np.save(fr'./results/label1/label_{i}.npy',labels)
